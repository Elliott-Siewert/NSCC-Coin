...
let http_port = process.env.HTTP_PORT || 3002;
let p2p_port = process.env.P2P_PORT || 6002;
let initialPeers = process.env.PEERS ? process.env.PEERS.split(',') : ['ws://172.16.144.133:6002'];
let sockets = [];
let blockchain = [];
let unspentTxOuts = [];
let transactionPool = [];
...
let createTransaction = (receiverAddress, amount, privateKey, aUnspentTxOuts) => {
    ...
    let myUnspentTxOutsA = aUnspentTxOuts.filter((uTxo) => {
        //console.log(uTxo.address + "==" + myAddress + (uTxo.address == myAddress));
    ...
    console.log("contructed tx" + JSON.stringify(tx));
    return tx;
...
let validateTxIn = (txIn, transaction, aUnspentTxOuts) => {
    ...
    let isValid = key.verify(transaction.id, txIn.signature);
    if(!isValid){
        console.log("failed valid test: " + JSON.stringify(txIn) + " " + transaction.id);
...
var initHttpServer = () => {
    ...
    app.post('/mineChain', (req, res) => {
        ...
        // }
        __miningControl = true;
        miningInterval = setInterval(generateNextBlockLoop, 100);
        res.send();
    });
    // app.post('/mineChain', (req, res) => {
    //     console.log("received request to mine chain");
    ...
    app.post('/sendTransaction' , (req, res) => {
        try{
            ...
            console.log("in post endpoint" + e.message);
